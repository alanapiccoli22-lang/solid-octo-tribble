...
from qgis.PyQt.QtWidgets import QDialog, QComboBox, QDoubleSpinBox, QMessageBox, QTableWidgetItem
...
class DemToTactileDialog(QDialog, FORM_CLASS):
    ...
    def __init__(self, iface, parent=None):
        ...
        :param iface: Interface do QGIS (QgisInterface) para interações com o sistema.
        """
        ...
        self.mExtentWidget.setOriginalExtent(QgsRectangle(), QgsProject.instance().crs())
        self.mExtentWidget.setCurrentExtent(QgsRectangle(), QgsProject.instance().crs())
        self.mExtentWidget.setOutputExtentFromOriginal()
        
        # Conectar sinal de mudança de camada raster para atualizar a extensão padrão
        self.mRasterLayerComboBox.layerChanged.connect(self.on_raster_layer_changed)
        ...
        # Carregar e exibir as logos na aba "Sobre" (Geolab e a startup AnthropicGeo)
        from qgis.PyQt.QtGui import QPixmap
        logo_geolab_path = os.path.join(os.path.dirname(__file__), 'logo_geolab.jpg')
        if os.path.exists(logo_geolab_path):
            self.labelLogo.setPixmap(QPixmap(logo_geolab_path))
            
        logo_anthropic_path = os.path.join(os.path.dirname(__file__), 'logo_anthropic.png')
    ...
    def setup_vector_table(self):
        ...
        self.mVectorTable.setHorizontalHeaderLabels([
            "Camada Vetorial", "Efeito Tátil", "Buffer (Metros)", "Altura/Prof. (mm)"
        ])
    ...
    def on_raster_layer_changed(self, layer):
        """Atualiza a extensão sugerida no widget de extensão ao trocar o raster.
        ...
        if layer:
            ...
            # Auto-preencher dimensões físicas baseadas no aspect ratio do raster
            extent = layer.extent()
    ...
    def add_vector_row(self):
        """Adiciona uma nova linha de configuração de camada vetorial na tabela."""
        ...
        buffer_spin.setValue(15.0) # 15 metros de buffer padrão para trilhas/rios
    ...
    def on_row_layer_changed(self, row, layer):
        """Ajusta a linha com base no tipo de geometria da camada (Linha vs Polígono)."""
        ...
        if not layer or not layer.isValid() or not effect_combo or not buffer_spin or not height_spin:
            return
        ...
        if geom_type == 2:
            # Polígonos representam áreas fechadas (como lagos/lagoas) e não precisam de buffer linear
            buffer_spin.setValue(0.0)
            buffer_spin.setEnabled(False)
            
            # Se for polígono, pre-selecionar o efeito de Massa d'Água Nivelada se não estiver selecionado
            current_effect = effect_combo.currentText()
            if "Lago/Massa" not in current_effect:
                effect_combo.setCurrentIndex(2) # Lago/Massa d'Água (Nivelado Fixo)
        else:
    ...
    def remove_selected_row(self):
        ...
        if selected_ranges:
            ...
            QMessageBox.warning(self, "Aviso", "Por favor, selecione uma linha da tabela para remover.")
    ...
    def on_generate_clicked(self):
        ...
        if not raster_layer:
            QMessageBox.critical(self, "Erro", "Selecione uma Camada Raster (MDE) válida.")
            return
            
        # 2. Validar extensão
        extent = self.mExtentWidget.outputExtent()
        if extent.isEmpty():
            QMessageBox.critical(self, "Erro", "A extensão de recorte selecionada é inválida ou está vazia.")
            return
        ...
        if width_mm <= 0 or height_mm <= 0 or res_mm <= 0:
            QMessageBox.critical(self, "Erro", "As dimensões e a resolução do modelo devem ser maiores que zero.")
            return

        # 4. Validar arquivo de destino
        output_file = self.mFileWidget.filePath()
        if not output_file:
            QMessageBox.critical(self, "Erro", "Selecione um local e nome de arquivo STL para salvar.")
            return
        ...
        # Executar processamento (bloqueante mas com processEvents para manter a UI viva)
        try:
            ...
            if success:
                ...
                self.iface.messageBar().pushSuccess(
                    "DEM to Tactile 3D", f"Modelo STL salvo com sucesso em {output_file}"
                )
            else:
                QMessageBox.critical(
                    self, "Erro", 
                    "Ocorreu um erro durante o processamento geométrico. Verifique os logs."
                )
