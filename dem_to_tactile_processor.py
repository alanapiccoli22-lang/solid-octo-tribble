...
class DemToTactileProcessor:
    """Motor de processamento geométrico para geração de modelos táteis 3D (STL)."""  # noqa: E501
    ...
    def run(self, raster_layer, extent, model_width_mm, model_height_mm,
        ...
        :param resolution_mm: Tamanho do pixel/grade no modelo físico em mm (ex: 0.5).  # noqa: E501
        :param base_height_m: Cota de elevação correspondente à base do bloco (Z=0) em metros.  # noqa: E501
        ...
        :param progress_callback: Função callback(percent_int, status_str) para atualizar a UI.  # noqa: E501
        """
        self.is_cancelled = False
        ...
        # Fatores de escala
        # scale_xy: escala horizontal adimensional (metros de terreno por metro
        # de modelo)
        scale_xy = width_geo / (model_width_mm / 1000.0)
        # scale_z: metros de terreno por milímetro de modelo físico (multiplica
        # por 1000 para converter M para MM)
        ...
        report_progress(
            10, "Amostrando MDE com interpolação bilinear contínua...")

        # 2. Amostragem do Raster (Interpolação Bilinear Contínua)
        provider = raster_layer.dataProvider()
        no_data_value = provider.sourceNoDataValue(1)

        try:
            full_ext = provider.extent()
            p_xsize = max(1, provider.xSize())
            p_ysize = max(1, provider.ySize())
            native_res_x = full_ext.width() / p_xsize
            native_res_y = full_ext.height() / p_ysize

            n_cols_native = max(2, int(np.ceil(width_geo / native_res_x)))
            n_rows_native = max(2, int(np.ceil(height_geo / native_res_y)))

            # Margem de segurança (2 pixels) para evitar artefatos de borda
            pad_x = 2 * native_res_x
            pad_y = 2 * native_res_y
            ...
            sub_cols = n_cols_native + 4
            sub_rows = n_rows_native + 4

            block = provider.block(1, sub_extent, sub_cols, sub_rows)
            if block and block.isValid():
                native_grid = block.as_numpy().astype(np.float32)

                mask = np.isnan(native_grid)
                if no_data_value is not None:
                    mask |= (native_grid == no_data_value)
                native_grid[mask] = base_height_m

                # Coordenadas geográficas dos centros das células do modelo
                target_x = np.linspace(
                    x_min + cell_size_x_geo / 2.0,
                    x_max - cell_size_x_geo / 2.0,
                    cols
                )
                target_y = np.linspace(
                    y_max - cell_size_y_geo / 2.0,
                    y_min + cell_size_y_geo / 2.0,
                    rows
                )

                sub_dx = sub_extent.width() / sub_cols
                sub_dy = sub_extent.height() / sub_rows

                col_indices = (
                    target_x - (sub_extent.xMinimum() + sub_dx / 2.0)) / sub_dx
                row_indices = (
                    (sub_extent.yMaximum() - sub_dy / 2.0) - target_y) / sub_dy

                R_mesh, C_mesh = np.meshgrid(
                    row_indices, col_indices, indexing='ij')

                # Order 1: Interpolação Bilinear Suave (elimina relevo
                # quadriculado / escadaria)
                grid_z = map_coordinates(
                    native_grid,
                    [R_mesh, C_mesh],
                    order=1,
                    mode='nearest'
        ...
        # Fallback de segurança se o bloco falhar
        if grid_z is None:
        ...
        # Garantir espessura mínima de segurança (0.2 mm) para não rasgar a
        # base
        grid_z_model = np.clip(grid_z_model, 0.2, None)

        # 3. Processar Sobreposições Vetoriais
        report_progress(
            45, "Preparando processamento das camadas vetoriais...")
        ...
        for idx, setting in enumerate(vector_settings):
            ...
            status_msg = f"Processando vetor {
                idx + 1}/{
                len(vector_settings)}: {
                layer.name()}..."
            report_progress(
                45 + int(15 * (idx / len(vector_settings))), status_msg)

            # Criar transformador de coordenadas se o CRS for diferente
            transform = None
            if layer.crs() != raster_crs:
                transform = QgsCoordinateTransform(
                    layer.crs(), raster_crs, project)

            # Requisitar feições. Se houver transformação, precisamos projetar a extensão geográfica do MDE  # noqa: E501
            # de volta para o CRS do vetor para que o filtro espacial de
            # feições funcione perfeitamente!
            request = QgsFeatureRequest()
            if transform:
                try:
                    inverse_transform = QgsCoordinateTransform(
                        raster_crs, layer.crs(), project)
                    vector_extent = inverse_transform.transformBoundingBox(
                        extent)
            ...
            # Para cada feição vetorial intersectando a área, aplicamos a deformação localmente  # noqa: E501
            # usando a otimização de bounding box para máxima performance em
            # Python (sem indexar todo pixel)
            for f in features:
                geom = f.geometry()
                ...
                # Obter a versão da geometria de busca (com buffer para linhas,
                # a própria para polígonos)
                ...
                # Obter o bounding box da geometria de busca e intersectar com
                # a extensão de recorte do MDE
                ...
                # Converter coordenadas geográficas da caixa envolvente em
                # índices da grade (r_start a r_end, c_start a c_end)
                c_start = max(
                    0, int(
                        (bbox_intersect.xMinimum() - x_min) / cell_size_x_geo))
                c_end = min(
                    cols - 1, int((bbox_intersect.xMaximum() - x_min) / cell_size_x_geo) + 1)  # noqa: E501

                r_start = max(
                    0, int(
                        (y_max - bbox_intersect.yMaximum()) / cell_size_y_geo))
                r_end = min(
                    rows - 1, int((y_max - bbox_intersect.yMinimum()) / cell_size_y_geo) + 1)  # noqa: E501

                # Loop restrito apenas aos pixels dentro da caixa envolvente da
                # feição
                        ...
                        if is_polygon:
                            if search_geom.contains(pt_geom):
                                ...
                                if "Nivelado Fixo" in effect:
                                    ...
                                    grid_z_model[r, c] = max(
                                        0.2, current_z - height_mm)
                            ...
                            if search_geom.contains(pt_geom):
                                ...
                                if d <= buffer_m:
                                    ...
                                    factor = (
                                        1.0 + math.cos(math.pi * (d / buffer_m))) / 2.0  # noqa: E501

                                    current_z = grid_z_model[r, c]
                                    if effect == "Extrudado (Positivo)":
                                        grid_z_model[r, c] = max(
                                            current_z, current_z + height_mm * factor)  # noqa: E501
                                    elif effect == "Escavado (Negativo)":
                                        grid_z_model[r, c] = min(current_z, max(  # noqa: E501
                                            0.2, current_z - height_mm * factor))  # noqa: E501

        report_progress(
            60, "Grade de relevo concluída. Iniciando triangulação da malha 3D...")  # noqa: E501

        # 4. Geração de Triângulos (Malha Sólida)
        # Reservar listas de triângulos para gravação em lote
        # Cada triângulo é armazenado como (normal_x, normal_y, normal_z, v1, v2, v3)  # noqa: E501
        ...
        # Função auxiliar para calcular normal simples (opcional, slicers
        # recalculam)
        ...
        # X cresce para a direita (colunas c), Y cresce para cima (linhas r decrescentes)  # noqa: E501
        # Mapeamento físico:
        # x_phys = c * dx
        # y_phys = (rows - 1 - r) * dy
        for r in range(rows - 1):
            ...
            if r % max(1, rows // 10) == 0:
                ...
                report_progress(
                    percent, f"Triangulando superfície: linha {r}/{rows}...")
        ...
        # 5. Escrever o arquivo STL binário de forma extremamente rápida e
        # robusta
        try:
            ...
            with open(output_path, 'wb') as f:
                ...
                header = b'DEM to Tactile 3D Plugin - Watertight Solid STL Output'  # noqa: E501
                ...
                # Cada triângulo tem: 3 floats (normal) + 9 floats (vértices) + 2 bytes (attribute byte count)  # noqa: E501
            ...
            report_progress(
                100, f"Modelo gerado com sucesso em: {output_path}")
            return True
